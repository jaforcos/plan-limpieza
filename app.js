...
(() => {
  ...
  // Identificador único por dispositivo (para evitar pisarse)
  const CLIENT_ID_KEY = 'planLimpieza_client_id';
  ...
  // Documento compartido por familia
  ...
  const loadState = () => {
    try {
      ...
      if (parseISO(merged.selectedDate) < PLAN_START) merged.selectedDate = toISO(PLAN_START);
      return merged;
    } catch {
      return defaultState();
    }
  ...
  // --- ✅ PASO 5: setDone guarda local + Firestore (con debounce) ---
  let pushTimer = null;
  ...
  async function pushStateToFirestore() {
    ...
    // Guardamos solo lo necesario para sincronizar:
    // done + marcas de control para no pisarse
    const payload = {
      done: state.done || {},
  ...
  const setDone = (dateISO, id, val) => {
    ensureBucket(state.done, dateISO)[id] = !!val;
    ...
    // debounce (evita escribir en Firestore por cada click muy rápido)
  ...
  const scheduledTasksForDate = (dateObj) => {
    ...
    if (dow === 0) {
      return (data.sunday || []).map((it, i) => ({
        iso, originalISO: iso, person: it.person, task: it.task, minutes: it.minutes,
        id: taskId('SUN', 'Domingo', it.person, i)
      }));
    }
    ...
    if (dow === 6) {
      ...
      return (block.items || []).map((it, i) => ({
        iso, originalISO: iso, person: it.person, task: it.task, minutes: it.minutes,
        id: taskId('SAT', 'Sábado', it.person, i, rot), rot, title: block.title
      }));
    }
  ...
  const sumMinutes = (tasks) => {
    let total = 0;
    for (const t of tasks) {
      const m = String(t.minutes || '').match(/\d+/);
  ...
  const renderDay = () => {
    ...
    pTitle.className = 'day-title';
    pTitle.innerHTML = '<h3>⏳ Pendientes</h3>';
    ...
    if (pending.length) {
      ...
      ok.className = 'task-card';
      ok.innerHTML = '<div class="task-text">🎉 ¡No hay pendientes!</div>';
      dayContent.appendChild(ok);
    }

    const tTitle = document.createElement('div');
    tTitle.className = 'day-title';
    tTitle.innerHTML = '<h3>✅ Tareas de hoy</h3>';
    ...
    if (scheduled.length) {
      ...
      none.className = 'task-card';
      none.innerHTML = '<div class="task-text">Hoy no hay tareas.</div>';
    ...
    if (d.getDay() === 6) {
      ...
      const rot = (state.rotationOverride === 'AUTO') ? autoRot : state.rotationOverride;
      rotationHint.textContent = (state.rotationOverride === 'AUTO')
        ? `🌀 Sábado: ${autoRot}`
        : `🌀 Sábado: ${rot} (auto: ${autoRot})`;
    } else {
  ...
  const render = () => {
    startText.value = toDMY(parseISO(state.selectedDate));
    rotationEl.value = state.rotationOverride;
    renderDay();
    renderExtras();
  };
  ...
  startText.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); commitDate(); startText.blur(); }
  });

  rotationEl.addEventListener('change', () => {
    state.rotationOverride = rotationEl.value;
    saveState();
    render();
  });
